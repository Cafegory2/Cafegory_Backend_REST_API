...
class CafeStudyServiceTest extends ServiceTest {
    ...
    void exception_case() {
        ...
        //then
        assertDoesNotThrow(() ->
                sut.createStudy(new MemberId(leader.getId()), now, cafeStudyCreateRequest.toStudyContent(), new CafeId(cafe.getId())));
    }
    ...
    void study_starts_1hours_after_now() {
        ...
        //then
        assertDoesNotThrow(
                () -> sut.createStudy(new MemberId(coordinator.getId()), timeUtil.now(), cafeStudyCreateRequest.toStudyContent(), new CafeId(cafe.getId())));
    }
    ...
    void study_starts_1hours_before_now() {
        ...
        assertThatThrownBy(
                () -> sut.createStudy(new MemberId(coordinator.getId()), timeUtil.now(),
                        cafeStudyCreateRequest.toStudyContent(), new CafeId(cafe.getId())))
    ...
    void includes_leader_in_participants() {
        ...
        //when
        StudyId studyId = sut.createStudy(new MemberId(coordinator.getId().getId()), now, study.getContent(), new CafeId(cafe.getId()));
    ...
    void study_can_start_between_cafe_business_hours(LocalDateTime start, LocalDateTime end) {
        ...
        //then
        assertDoesNotThrow(() -> sut.createStudy(new MemberId(leader.getId()), now, studyOnceCreateRequest.toStudyContent(), new CafeId(cafe.getId())));
    }
