...
public class BusinessHourOpenCheckerTest {
    ...
    void determines_if_it_is_during_business_hours(LocalDateTime now) {
        ...
        List<BusinessHourEntity> businessHourEntities = List.of(
                aBusinessHour().withDayOfWeek(MONDAY)
                        .withOpeningTime(9, 0)
    ...
    void open_24Hours(LocalDateTime now) {
        ...
        List<BusinessHourEntity> businessHourEntities = List.of(
                businessHour24.but().withDayOfWeek(MONDAY).build(),
                businessHour24.but().withDayOfWeek(TUESDAY).build()
        );
        //when
        boolean isOpen = sut.checkWithBusinessHours(businessHourEntities, now);
        //then
    ...
    void open_until_early_morning_the_next_day(LocalDateTime now, boolean expected) {
        ...
        List<BusinessHourEntity> businessHourEntities = List.of(
                businessHour9To26.but().withDayOfWeek(MONDAY).build(),
                businessHour9To26.but().withDayOfWeek(TUESDAY).build()
        );
        //when
        boolean isOpen = sut.checkWithBusinessHours(businessHourEntities, now);
        //then
    ...
    void business_hours_are_different_each_day(LocalDateTime now,
        ...
        List<BusinessHourEntity> businessHourEntities = List.of(
                businessHour9To22.but().withDayOfWeek(MONDAY).build(),
                businessHour9To24.but().withDayOfWeek(FRIDAY).build(),
                businessHour24.but().withDayOfWeek(SATURDAY).build(),
                businessHour0To22.but().withDayOfWeek(SUNDAY).build()
        );
        //when
        boolean isOpen = sut.checkWithBusinessHours(businessHourEntities, now);
        //then
