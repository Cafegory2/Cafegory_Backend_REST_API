...
class CafeStudyQueryRepositoryTest extends JpaTest {
	...
	void find_cafe_studies_by_keyword(String keyword, int expected) {
		...
		SliceResponse<CafeStudyEntity> result = sut.findCafeStudies(
			...
			createSearchCriteria(keyword, null, null, null),
			createCafeTags(null),
			createPage(0, 10)
		);
		//then
	...
	void find_cafe_studies_by_start_date_time(
		...
		SliceResponse<CafeStudyEntity> result = sut.findCafeStudies(
			...
			createSearchCriteria("강남", specificDate, null, null),
			createCafeTags(null),
			createPage(0, 10)
		);
		//then
	...
	void find_cafe_studies_by_cafe_study_tag(CafeStudyTagType type, int expected) {
		...
		CafeStudyEntity cafeStudy1 = cafeStudySaveHelper.saveCafeStudy(cafe1, member, startDateTime.plusHours(2),
			startDateTime.plusHours(4));
		cafeStudyCafeStudyTagSaveHelper.saveCafeStudyCafeStudyTag(cafeStudy1, cafeStudyTag1);
		CafeStudyEntity cafeStudy2 = cafeStudySaveHelper.saveCafeStudy(cafe1, member, startDateTime.plusHours(5),
			startDateTime.plusHours(7));
		cafeStudyCafeStudyTagSaveHelper.saveCafeStudyCafeStudyTag(cafeStudy2, cafeStudyTag2);

		CafeStudyEntity cafeStudy3 = cafeStudySaveHelper.saveCafeStudy(cafe2, member, startDateTime.plusHours(8),
		...
		SliceResponse<CafeStudyEntity> result = sut.findCafeStudies(
			...
			createSearchCriteria("강남", null, type, null),
			createCafeTags(null),
			createPage(0, 10)
		);
		//then
	...
	void find_cafe_studies_by_cafe_tag(CafeTagType type, int expected) {
		...
		SliceResponse<CafeStudyEntity> result = sut.findCafeStudies(
			...
			createSearchCriteria("강남", null, null, null),
			createCafeTags(List.of(type)),
			createPage(0, 10)
		);
		//then
		assertThat(result.getContent().size()).isEqualTo(expected);
	...
	void find_cafe_studies_by_cafe_tags(CafeTagType type1, CafeTagType type2, int expected) {
		...
		SliceResponse<CafeStudyEntity> result = sut.findCafeStudies(
			...
			createSearchCriteria("강남", null, null, null),
			createCafeTags(List.of(type1, type2)),
			createPage(0, 10)
		);
		//then
		assertThat(result.getContent().size()).isEqualTo(expected);
	...
	void find_cafe_studies_by_member_communication(MemberComms memberComms, int expected) {
		...
		SliceResponse<CafeStudyEntity> result = sut.findCafeStudies(
			...
			createSearchCriteria("강남", null, null, memberComms),
			createCafeTags(null),
			createPage(0, 10)
		);
		//then
	...
	void show_available_cafe_studies_first() throws Exception {
		...
		CafeStudyEntity cafeStudy1 = cafeStudySaveHelper.saveCafeStudy(cafeEntity1, member, startDateTime.plusHours(8),
			startDateTime.plusHours(10));
		Thread.sleep(1000);
		CafeStudyEntity finishedCafeStudy2 = cafeStudySaveHelper.saveFinishedCafeStudy(cafeEntity1, member,
			startDateTime.plusHours(2), startDateTime.plusHours(4));
		Thread.sleep(1000);
		CafeStudyEntity cafeStudy3 = cafeStudySaveHelper.saveCafeStudy(cafeEntity1, member, startDateTime.plusHours(5),
			startDateTime.plusHours(7));
		Thread.sleep(1000);
		CafeStudyEntity finishedCafeStudy4 = cafeStudySaveHelper.saveFinishedCafeStudy(cafeEntity1, member,
		...
		assertThat(content)
			...
			.containsExactly(cafeStudy3.getCreatedDate(), cafeStudy1.getCreatedDate(),
				finishedCafeStudy4.getCreatedDate(), finishedCafeStudy2.getCreatedDate());
	}
	...
	void find_cafe_studies_with_first_page() {
		...
		for (int i = 0; i < 6; i++) {
			cafeStudySaveHelper.saveCafeStudy(cafeEntity1, member, startDateTime.plusHours(i),
				startDateTime.plusHours(i + 1));
		}
		//when
	...
	void find_cafe_studies_by_many_different_filtering(
		...
		SliceResponse<CafeStudyEntity> result = sut.findCafeStudies(
			...
			createSearchCriteria("강남", specificDate, cafeStudyTagType, memberComms),
			createCafeTags(cafeTagTypes),
			createPage(0, 5)
		);
		assertThat(result.getContent().size()).isEqualTo(expected);
	}
